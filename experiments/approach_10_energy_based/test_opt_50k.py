"""50K test of optimized energy-based retrieval."""
...
print("\n" + "="*70)
print("  APPROACH 10 OPTIMIZED @ 50K — Verify generic fix preserves quality")
print("="*70)

corpus = generate_realistic_corpus(50000)
print(f"\n  Generated {len(corpus):,} docs")
...
print(f"    Recall:      {sum(recall_scores)/len(recall_scores)*100:.1f}%")
...
print(f"    avg threshold-pass concepts: {sum(thresh_stats)/len(thresh_stats):.1f}/14")

print(f"\n  COMPARISON @ 50K:")
print(f"    Original:    P@10=97%, NDCG=0.969, p50=5.6ms, p95=22.6ms")
print(f"    Optimized:   P@10={sum(p10_scores)/len(p10_scores)*100:.0f}%, NDCG={sum(ndcg_scores)/len(ndcg_scores):.3f}, p50={latencies[len(latencies)//2]:.2f}ms, p95={latencies[int(len(latencies)*0.95)]:.2f}ms")
