"""
5K Scale Test — Approach 9
===========================

Testing probabilistic neighborhoods + 5-component ranking at
standard benchmark scale (5,000 documents).

Compares to:
- Approach 3 at 5K: P@10=58%, Recall=0.1%
- Approach 9 at 1K: P@10=87%, Recall=81%

Question: Does Approach 9's performance hold at scale?
"""
...
from scbs.distance import build_idf, sentence_weight, weighted_matrix_distance
...
# Import Approach 9's probabilistic components from test_approach_9.py
sys.path.insert(0, _HERE)
from test_approach_9 import (
    compute_cluster_centroids,
    compute_neighborhood_memberships,
    ProbabilisticStore
)
...
def generate_realistic_corpus(n_docs=5000):
    ...
    for i in range(n_docs):
        ...
        # Use Template which is more forgiving with missing variables
        template = Template(template_str)
        
        # Create a safe substitution dict with defaults
        subs = {
        ...
        # Safe substitute - leaves unmatched variables alone
        doc = template.safe_substitute(subs)
...
def main():
    print("\n" + "="*70)
    print("  APPROACH 9 @ 5K SCALE")
    print("  Probabilistic Neighborhoods + 5-Component Ranking")
    print("="*70)
    ...
    # Build index
    print("\n  Building Approach 9 index with expanded neighborhoods...")
    encoder = BlueprintEncoder("approach9_5k_unknown.json")
    store = ProbabilisticStore(n_clusters=14, top_k_neighborhoods=5)  # Expanded from 3 to 5
    ...
    for query_text, keywords in queries:
        ...
        results, _ = store.search(
            qr, query_text,
            top_k=100,
            threshold=9000  # high threshold since scoring is different
        )
    ...
    latencies.sort()
    print(f"\n{'='*70}")
    print(f"  RESULTS — Approach 9 at 5K Scale")
    print(f"{'='*70}")
    print(f"\n  Scale:")
    ...
    print(f"\n  Comparison:")
    print(f"    Approach 3 @ 1K:  P@10=96%,  Recall(F1)=23%,  p50=0.4ms")
    print(f"    Approach 3 @ 5K:  P@10=58%,  Recall=0.1%,     p50=3.7ms")
    print(f"    Approach 9 @ 1K:  P@10=87%,  Recall=81%,      p50=8.2ms")
    print(f"    Approach 9 @ 5K:  P@10={sum(p10_scores)/len(p10_scores)*100:.0f}%,  Recall={sum(recall_scores)/len(recall_scores)*100:.0f}%,      p50={latencies[len(latencies)//2]:.1f}ms")
    
    # Clean up
    if os.path.exists("approach9_5k_unknown.json"):
        os.remove("approach9_5k_unknown.json")
